...
class MinimalSubscriber : public ZcNode
{
  ...
  private:
    ...
    void topic_callback(const std_msgs::msg::String & msg, std::function<void()> clear)
    {
      ShmImage * img = shm->find<ShmImage>(msg.data.c_str()).first;
      if (img == nullptr) {
        RCLCPP_ERROR(this->get_logger(), "Failed to find shared memory object: %s", msg.data.c_str());
        clear();
        return;
      }
